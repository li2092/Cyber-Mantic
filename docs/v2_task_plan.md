# 赛博玄数 V2 版本 - 任务规划书

> 更新时间：2026-01-08
> 负责人：Claude + 大魔王
> 状态：阶段五完成 ✅

---

## 一、版本目标

V2版本主要目标：
1. **界面重构**：左侧导航栏 + 问道右侧面板优化
2. **功能增强**：MBTI适配、仲裁系统、动态验证、时辰智能处理
3. **API拓展**：更多厂商支持 + 环节级自定义
4. **体验优化**：快速分析渐进展示

---

## 二、功能模块清单

### 2.1 界面重构

| 功能 | 描述 | 优先级 | 复杂度 |
|------|------|--------|--------|
| 左侧导航栏 | 替代顶部标签页，支持收起/展开 | P0 | 高 |
| Logo区域 | 图标+中文名+英文名 | P0 | 低 |
| 导航项图标 | 每个导航项配图标 | P0 | 低 |
| 问道右侧面板 | 进度→快速结论→基本信息 | P0 | 中 |
| 右侧面板拖拽 | 可拖拽调整宽度 | P1 | 中 |
| 输入框等宽 | 与对话区域等宽 | P0 | 低 |

### 2.2 核心功能增强

| 功能 | 描述 | 优先级 | 复杂度 |
|------|------|--------|--------|
| MBTI适配矩阵 | 16×8完整适配度表 | P1 | 低 |
| 仲裁系统 | 冲突时引入第三方理论裁决 | P1 | 中 |
| 动态回溯验证 | AI生成3个针对性验证问题 | P1 | 中 |
| 快速分析反馈 | 理论完成即展示，渐进式 | P0 | 中 |
| 时辰智能处理 | 三方案混合（三柱/并行/验证） | P2 | 高 |
| 真太阳时校正 | 推演模块，根据经度+分钟计算 | P2 | 中 |

### 2.3 API拓展

| 功能 | 描述 | 优先级 | 复杂度 |
|------|------|--------|--------|
| 新增厂商支持 | 通义千问、豆包、百川、智谱 | P1 | 中 |
| 环节级配置 | 每个AI调用环节可单独配置API | P1 | 高 |
| 配置持久化 | 环节配置保存到设置文件 | P1 | 低 |
| 设置界面 | 分环节自定义UI | P1 | 中 |

---

## 三、技术设计要点

### 3.1 左侧导航栏

```
结构：
├── SidebarWidget
│   ├── LogoHeader（80-100px高）
│   │   ├── Logo图标（可用现有app_icon.png）
│   │   ├── 中文名：赛博玄数
│   │   └── 英文名：Cyber Mantic（小字）
│   ├── NavItemList
│   │   ├── NavItem: 问道（💬）
│   │   ├── NavItem: 推演（📊）
│   │   ├── NavItem: 典籍（📚）
│   │   ├── NavItem: 洞察（👁）
│   │   ├── NavItem: 历史（📜）
│   │   └── NavItem: 设置（⚙️）
│   ├── Separator
│   └── NavItem: 关于（ℹ️）底部固定

收起/展开：
- 展开宽度：180px
- 收起宽度：60px（只显示图标）
- 切换按钮：在Logo区域右下角
```

### 3.2 问道右侧面板

```
结构（从上到下）：
├── ProgressSection（进度区）
│   ├── 进度条（ProgressBar）
│   └── 阶段文字（"阶段4: 结果确认"）
├── QuickResultSection（快速结论区）
│   ├── QuickResultCard: 小六壬
│   ├── QuickResultCard: 八字
│   ├── QuickResultCard: 奇门遁甲
│   └── ...
└── InfoSection（基本信息区）
    ├── 问题信息
    ├── 时辰状态
    ├── 性别
    └── MBTI

宽度：200-300px，可拖拽调整
```

### 3.3 快速结论卡片状态

```
状态枚举：
- WAITING（等待中）：灰色边框，⬚图标
- RUNNING（进行中）：蓝色边框+动画，⏳图标
- COMPLETED_GOOD（完成-吉）：绿色边框，✅图标
- COMPLETED_BAD（完成-凶）：红色边框，⚠️图标
- COMPLETED_NEUTRAL（完成-平）：橙色边框，➖图标
- ERROR（出错）：灰色，❌图标

信号机制：
- ConversationWorker新增信号：
  - theory_started(theory_name: str)
  - theory_completed(theory_name: str, result: dict)
  - quick_result(theory_name: str, summary: str, judgment: str)
```

### 3.4 MBTI适配矩阵

```python
# 完整16×8矩阵
MBTI_THEORY_MATRIX = {
    "INTJ": {"八字": 0.8, "奇门遁甲": 0.9, "梅花易数": 0.7, "小六壬": 0.6, "测字术": 0.5, "六爻": 0.7, "紫微斗数": 0.8, "大六壬": 0.7},
    "INTP": {"八字": 0.9, "奇门遁甲": 0.7, "梅花易数": 0.8, "小六壬": 0.5, "测字术": 0.6, "六爻": 0.8, "紫微斗数": 0.9, "大六壬": 0.6},
    # ... 其他14个类型
}

# 在theory_selector.py中实现查表逻辑
def calculate_mbti_matching(mbti_type: str, theory: str) -> float:
    if mbti_type is None or mbti_type not in MBTI_THEORY_MATRIX:
        return 1.0  # 未提供MBTI不影响选择
    return MBTI_THEORY_MATRIX[mbti_type].get(theory, 0.7)
```

### 3.5 仲裁系统

```python
class ArbitrationSystem:
    """仲裁系统：冲突时引入第三方理论裁决"""

    # 仲裁理论优先级（按问题类型）
    ARBITRATION_PRIORITY = {
        "事业": ["六爻", "梅花易数", "小六壬"],
        "感情": ["测字术", "梅花易数", "六爻"],
        "财运": ["六爻", "奇门遁甲", "小六壬"],
        "健康": ["六爻", "小六壬", "梅花易数"],
        "决策": ["奇门遁甲", "六爻", "大六壬"],
    }

    def request_arbitration(self, conflict, question_type, used_theories) -> str:
        """请求仲裁理论"""
        priority_list = self.ARBITRATION_PRIORITY.get(question_type, ["六爻", "梅花易数"])
        for theory in priority_list:
            if theory not in used_theories:
                return theory
        return None

    def execute_arbitration(self, arbitration_theory, conflict_theories, user_input):
        """执行仲裁分析"""
        # 1. 计算仲裁理论
        # 2. 比较仲裁结果与冲突双方
        # 3. 返回最终判断
        pass
```

### 3.6 动态回溯验证

```python
class DynamicVerificationGenerator:
    """动态生成验证问题"""

    async def generate_questions(self, user_info: dict, analysis_results: dict) -> List[dict]:
        """
        根据用户信息和分析结果生成3个针对性验证问题

        策略：
        1. 基于用户年龄计算关键年份（婚姻、事业转折等）
        2. 基于问题类别选择相关领域
        3. 基于理论预测的"应验年"生成验证
        """
        prompt = f"""
        用户信息：{json.dumps(user_info, ensure_ascii=False)}
        分析结果：{json.dumps(analysis_results, ensure_ascii=False)}

        请生成3个回溯验证问题，要求：
        1. 问题涉及过去3-5年内的具体事件
        2. 与用户咨询的问题类别相关
        3. 答案明确（是/否/大致时间）
        4. 用于验证分析准确性

        返回JSON格式：
        [
            {"question": "...", "type": "yes_no|year|event", "purpose": "..."},
            ...
        ]
        """
        # 调用AI生成
        response = await self.api_manager.call_api(prompt=prompt, task_type="回溯验证生成")
        return json.loads(response)
```

### 3.7 时辰智能处理（混合方案）

```python
class ShichenHandler:
    """时辰状态统一处理器"""

    class Status(Enum):
        KNOWN_EXACT = "known_exact"      # 确切知道
        KNOWN_RANGE = "known_range"      # 知道范围（问道专用）
        UNCERTAIN = "uncertain"          # 不确定是否准确
        UNKNOWN = "unknown"              # 完全不知道

    def process(self, user_input, life_events=None):
        """处理时辰并返回分析策略"""

        if self.status == self.Status.KNOWN_EXACT:
            return {"strategy": "full_analysis", "confidence": 0.95}

        elif self.status == self.Status.KNOWN_RANGE:
            # 方案二：范围内并行计算
            candidates = self._get_range_candidates()
            return {"strategy": "parallel", "candidates": candidates, "confidence": 0.7}

        elif self.status == self.Status.UNCERTAIN:
            # 方案三：事件验证
            if life_events:
                validation = self._validate_with_events(life_events)
                return {"strategy": "validated", "confidence": validation["confidence"]}
            return {"strategy": "degraded", "confidence": 0.6}

        else:  # UNKNOWN
            # 方案一：三柱降级 + 不依赖时辰的理论优先
            return {
                "strategy": "degraded",
                "bazi_mode": "three_pillars",
                "ziwei_mode": "disabled",
                "enabled_theories": ["六爻", "梅花易数", "小六壬", "测字术", "奇门遁甲", "大六壬"],
                "confidence": 0.5
            }
```

### 3.8 API环节级配置

```python
# 配置结构
API_TASK_CONFIG = {
    "global": {
        "primary": "claude",
        "fallback_order": ["deepseek", "kimi", "gemini"],
    },
    "task_overrides": {
        "综合报告解读": {"api": "claude", "model": "claude-3-opus"},
        "快速交互问答": {"api": "kimi", "model": "kimi-k2-turbo"},
        "冲突解决分析": {"api": "deepseek", "model": "deepseek-reasoner"},
        "回溯验证生成": {"api": "gemini", "model": "gemini-pro"},
        "理论解读": None,  # None表示使用全局配置
    }
}

# 支持的API厂商
SUPPORTED_APIS = {
    "claude": {"vendor": "Anthropic", "models": ["claude-3-opus", "claude-3-sonnet", "claude-3-haiku"]},
    "deepseek": {"vendor": "DeepSeek", "models": ["deepseek-chat", "deepseek-reasoner"]},
    "kimi": {"vendor": "Moonshot", "models": ["kimi-k2", "kimi-k2-turbo"]},
    "gemini": {"vendor": "Google", "models": ["gemini-pro", "gemini-ultra"]},
    "qwen": {"vendor": "阿里通义", "models": ["qwen-max", "qwen-plus", "qwen-turbo"]},
    "doubao": {"vendor": "字节豆包", "models": ["doubao-pro", "doubao-lite"]},
    "baichuan": {"vendor": "百川", "models": ["baichuan-4", "baichuan-3-turbo"]},
    "glm": {"vendor": "智谱清言", "models": ["glm-4", "glm-4-flash", "glm-3-turbo"]},
}
```

---

## 四、开发计划

### 阶段一：界面重构（2周）✅ 已完成

| 任务 | 描述 | 工时 | 依赖 | 状态 |
|------|------|------|------|------|
| 1.1 左侧导航栏组件 | SidebarWidget + NavItem | 2天 | 无 | ✅ |
| 1.2 Logo区域 | LogoHeader组件 | 0.5天 | 无 | ✅ |
| 1.3 MainWindow重构 | 替换TabWidget为左侧导航 | 1.5天 | 1.1 | ✅ |
| 1.4 收起/展开功能 | 汉堡菜单按钮+状态切换 | 1天 | 1.3 | ✅ |
| 1.5 问道右侧面板 | ProgressPanel + QuickResultCard + InfoPanel | 2天 | 1.3 | ✅ |
| 1.6 拖拽调整宽度 | QSplitter优化 | 0.5天 | 1.5 | ✅ |
| 1.7 输入框等宽 | 布局调整 | 0.5天 | 1.5 | ✅ |
| 1.8 主题适配 | 深色主题样式 | 1天 | 1.1-1.7 | ✅ |
| 1.9 聊天气泡优化 | AI头像上方+用户紫色气泡+打字动画 | 0.5天 | 1.5 | ✅ |
| 1.10 输入框配色 | 深色主题友好配色 | 0.5天 | 1.8 | ✅ |

### 阶段二：快速分析反馈 + MBTI（2周）✅ 已完成

| 任务 | 描述 | 工时 | 依赖 | 状态 |
|------|------|------|------|------|
| 2.1 Worker信号机制 | 新增theory_started/completed信号 | 1天 | 阶段一 | ✅ |
| 2.2 快速结论卡片交互 | 状态切换、动画效果 | 1天 | 1.5 | ✅ |
| 2.3 渐进展示逻辑 | 理论完成即更新UI | 1.5天 | 2.1, 2.2 | ✅ |
| 2.4 MBTI矩阵数据 | 补充完整16×8矩阵 | 0.5天 | 无 | ✅ |
| 2.5 MBTI匹配算法 | theory_selector.py实现 | 0.5天 | 2.4 | ✅ |
| 2.6 MBTI对报告影响 | 个性化表达调整 | 1天 | 2.5 | ✅ |

### 阶段三：动态验证 + API拓展（2周）✅ 已完成

| 任务 | 描述 | 工时 | 依赖 | 状态 |
|------|------|------|------|------|
| 3.1 动态问题生成 | DynamicVerificationGenerator | 1.5天 | 无 | ✅ |
| 3.2 验证问题UI | 对话中展示验证问题 | 1天 | 3.1 | ✅ |
| 3.3 验证结果处理 | 更新置信度逻辑 | 1天 | 3.2 | ✅ |
| 3.4 新增API客户端 | qwen/doubao/baichuan/glm + OpenRouter | 2天 | 无 | ✅ |
| 3.5 环节配置数据结构 | TaskRouter + TaskConfig | 0.5天 | 3.4 | ✅ |
| 3.6 配置持久化 | 保存到api_task_config.json | 0.5天 | 3.5 | ✅ |

### 阶段四：仲裁系统 + API配置UI（2周）✅ 已完成

| 任务 | 描述 | 工时 | 依赖 | 状态 |
|------|------|------|------|------|
| 4.1 ArbitrationSystem | 仲裁系统核心逻辑 | 1.5天 | 无 | ✅ |
| 4.2 仲裁触发逻辑 | 冲突解决器集成 | 1天 | 4.1 | ✅ |
| 4.3 仲裁结果展示 | UI展示仲裁过程 | 1天 | 4.2 | ⏳ 待完善 |
| 4.4 API设置界面 | 全局+分环节配置UI | 2天 | 3.5 | ✅ |
| 4.5 连接测试功能 | 测试各API连通性 | 0.5天 | 4.4 | ✅ |

### 阶段五：时辰智能处理 + 真太阳时（3周）✅ 已完成

| 任务 | 描述 | 工时 | 依赖 | 状态 |
|------|------|------|------|------|
| 5.1 ShichenHandler | 时辰状态处理器 | 1.5天 | 无 | ✅ |
| 5.2 三柱分析模式 | 八字降级计算 | 1.5天 | 5.1 | ✅ |
| 5.3 并行计算模式 | 多时辰候选分析 | 2天 | 5.1 | ✅ |
| 5.4 事件验证推断 | 基于历史事件缩小范围 | 2天 | 5.1 | ⏳ TODO stub |
| 5.5 NLP解析扩展 | 识别known_range状态 | 1天 | 5.1 | ✅ |
| 5.6 真太阳时校正 | 推演模块计算逻辑 | 1.5天 | 无 | ✅ |
| 5.7 推演UI调整 | 经度输入+提示 | 0.5天 | 5.6 | ⏳ 待实现 |
| 5.8 FlowGuard核心 | 流程监管模块设计 | 1天 | 无 | ✅ |
| 5.9 验证器实现 | 各阶段输入验证器 | 0.5天 | 5.8 | ✅ |
| 5.10 进度展示 | 类todo进度UI | 0.5天 | 5.8 | ✅ |
| 5.11 集成到问道 | ConversationService集成 | 1天 | 5.8 | ✅ |
| 5.12 判断节点梳理 | 18个节点AI+代码机制分析 | 0.5天 | 5.11 | ✅ |
| 5.13 AI增强任务类型 | 统一INPUT_ENHANCE配置 | 0.5天 | 5.12 | ✅ |
| 5.14 时辰确定性AI | NLP时间表达AI分析 | 0.5天 | 5.13 | ✅ |
| 5.15 问题类型AI | 问题分类AI识别 | 0.5天 | 5.13 | ✅ |
| 5.16 吉凶判断AI | 理论结果AI判断 | 0.5天 | 5.13 | ✅ |
| 5.17 事件时辰推断 | narrow_by_event实现 | 1天 | 5.13 | ✅ |

### 阶段六：测试优化（1周）

| 任务 | 描述 | 工时 | 依赖 |
|------|------|------|------|
| 6.1 界面测试 | 各模块UI测试 | 1天 | 阶段一 |
| 6.2 功能测试 | 完整流程测试 | 2天 | 阶段二-五 |
| 6.3 API测试 | 多厂商切换测试 | 1天 | 阶段三-四 |
| 6.4 性能优化 | 加载速度、响应时间 | 1天 | 全部 |

---

## 五、开发顺序总览

```
第1-2周：阶段一（界面重构）
    ↓
第3-4周：阶段二（快速分析反馈 + MBTI）
    ↓
第5-6周：阶段三（动态验证 + API拓展）
    ↓
第7-8周：阶段四（仲裁系统 + API配置UI）
    ↓
第9-11周：阶段五（时辰智能处理 + 真太阳时）
    ↓
第12周：阶段六（测试优化）
```

**总计：约12周（3个月）**

---

## 六、关键文件清单

### 新增文件（V2已完成）

```
cyber_mantic/ui/
├── components/
│   ├── sidebar.py              # 左侧导航栏 ✅
│   ├── nav_item.py             # 导航项 ✅
│   └── __init__.py             # 组件模块 ✅
├── widgets/
│   ├── quick_result_card.py    # 快速结论卡片 ✅
│   ├── chat_widget.py          # 聊天组件（重构）✅
│   ├── markdown_webview.py     # Markdown渲染 ✅
│   ├── verification_widget.py  # 验证问题组件 ✅
│   └── api_settings_widget.py  # API设置组件 ✅

cyber_mantic/core/
├── arbitration_system.py       # 仲裁系统 ✅
├── shichen_handler.py          # 时辰处理器 ✅
├── dynamic_verification.py     # 动态验证生成 ✅
├── theory_selector.py          # 理论选择器（含MBTI矩阵）✅
├── flow_guard.py               # 流程监管模块 ✅ [新增]
└── __init__.py                 # 核心模块导出 ✅

cyber_mantic/api/
├── clients/
│   ├── qwen_client.py          # 通义千问 ✅
│   ├── doubao_client.py        # 字节豆包 ✅
│   ├── baichuan_client.py      # 百川 ✅
│   ├── glm_client.py           # 智谱清言 ✅
│   └── openrouter_client.py    # OpenRouter聚合 ✅
├── task_router.py              # 环节级路由 ✅
└── unified_client.py           # 统一客户端 ✅

cyber_mantic/utils/
└── time_utils.py               # 时间工具（含真太阳时）✅

cyber_mantic/services/conversation/
└── nlp_parser.py               # NLP解析器（V2增强）✅

cyber_mantic/theories/bazi/
└── calculator.py               # 八字计算器（含三柱/并行）✅
```

### 修改文件

```
cyber_mantic/ui/
├── main_window.py              # 重构为左侧导航布局
└── tabs/
    └── ai_conversation_tab.py  # 适配新布局

cyber_mantic/core/
├── theory_selector.py          # 添加MBTI矩阵
└── conflict_resolver.py        # 集成仲裁系统

cyber_mantic/services/
└── conversation_service.py     # 集成动态验证

cyber_mantic/api/
└── manager.py                  # 支持环节级配置

cyber_mantic/theories/bazi/
└── calculator.py               # 三柱分析模式
```

---

## 七、风险与注意事项

1. **界面重构风险**：改动较大，需要充分测试
2. **API兼容性**：新增厂商需要处理不同的错误码和限流策略
3. **时辰处理复杂度**：并行计算模式可能增加API调用成本
4. **向后兼容**：确保老用户数据不丢失

---

## 八、待确认事项

- [x] 左侧导航栏收起/展开：要
- [x] Logo区域高度：美观为准（80-100px）
- [x] 导航项图标：要
- [x] 右侧面板拖拽：可以
- [x] 推演模块时辰选项：保持3个，不改4个
- [x] 真太阳时校正：只在推演模块
- [ ] Logo图标使用现有还是新设计？
- [ ] 深色主题是否同步开发？

---

*此文档将随开发进度持续更新*
